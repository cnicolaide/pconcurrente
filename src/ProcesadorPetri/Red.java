...
public class Red {
	...
	public boolean ejecutar(int[][] iDisparo, boolean editar) {
		...
		int aux = 0;// si aux es 0 significa que no hay transición
					// sensibilizada
	...
	public ArrayList<Integer> ejecutar(int[][] iDisparo) {
		...
		if (isTimetable) { // si es con tiempo, se agrega a la AND el vector
							// temporal y las sensibilizadas
			verificarTiempos();
			...
			oMand = oDisparo.AND(oFdeH); // sino no es con tiempo, la AND será
											// con el disparo y la H
		}
		int aux = 0;
		for (int i = 0; i < oMand.getFilCount(); i++) {
			for (int j = 0; j < oMand.getColCount(); j++) {
		...
		if (isTimetable) {/*
							 * Verifico si alguna transicion que estaba
							 * sensibilizada por tiempo se ejecutó para
	...
	private void setTiempos(int[][] iTiempoIntervalo) {
		...
		iTiempo = new int[1][iTiempoIntervalo.length];// creo el iVector para
														// devolver a la red.
		for (int i = 0; i < iTiempo[0].length; i++) {// seteo en 1 las
														// transiciones sin
														// tiempo.
	...
	public void verificarTiempos() {
		// verificar si una transicion deberia estar en un estado y no lo esta.
		// En ese caso resetear el contador.
			...
			for (int i = 0; i < AND.getFilCount(); i++) {
				for (int j = 0; j < AND.getColCount(); j++) {
					if (AND.getVal(i, j) == 1) {
						...
						if (print) {
							Escritor.Instance().Escribir("RED",
									"Disparo Automatico:  " + new Matriz(iDisparo).toString());
							Escritor.Instance().Escribir("RED",
									"Marcado Actual:  " + new Matriz(iMarcadoActual).toString());
						}
	...
	private void printEstados() {
		if (print) {
			...
			if (isTimetable) {
				...
				Escritor.Instance().Escribir("RED",
						"Transiciones Sensibilizadas Temporales: " + getSensibilizadasTemporales().toString());
			}
			if (iAutomaticas != null) {
